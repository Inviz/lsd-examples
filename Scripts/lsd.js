...
Class.mixin = function(instance, klass) {
  ...
  if (proto.onMix) proto.onMix.call(instance, instance);
}
...
Class.unmix = function(instance, klass) {
  ...
  if (proto.onUnmix) proto.onUnmix.call(instance, instance);
}
...
LSD.Command = new Class({
  ...
  attach: function(widget) {
    ...
    console.log(widget.events, 'attach command?')
    var events = widget.events._command = {}, self = this;
    for (var name in states) (function(name, state) {
...
LSD.Module.Events = new Class({
  options: {
    events: {
      _events: {
        'inject': function(widget) {
          if (this.events.parent && widget instanceof LSD.Widget) widget.addEvents(this.events.parent);
        },    
        'dispose': function(widget) {
          if (this.events.parent && widget instanceof LSD.Widget) widget.removeEvents(this.events.parent);
        }
      }
    }
  },
  ...
  addEvents: function(events) {
    for (var i in events) { 
      if (events[i].call) { //stick to old behaviour when key: function object is passed
        Events.prototype.addEvents.call(this, events);
        return false;
      } else {
        events = Hash.extend({}, events), convert = LSD.Module.Events.target;
        for (name in events) {
          ...
          if (target != this) {
            ...
            target.addEvents(subset);
          } else Events.prototype.addEvents.call(this, subset);
          delete events[name];
        }
  ...
  addStateEvents: function(events) {
    var Events = LSD.States.Events;
    for (var name in events) {
      var state = LSD.Events.Positive[name], positive = state;
      if (!state) state = LSD.Events.Negative[name]
      if (!state) continue;
      ...
      var setting = LSD.States.Known[state];
      this.addEvent(setting[+!positive], add);
      this.addEvent(setting[+!!positive], remove);
      delete events[name]
    }
  },
    ...
    for (var i in events) { 
      if (events[i].call) { //stick to old behaviour when key: function object is passed
        ...
        for (name in events) {
          ...
          if (target != this) {
            if (target == true) target = this;
            target.removeEvents(subset);
          } else Events.prototype.removeEvents.call(this, subset);
          delete events[name];
        }
      };  
      break;
...
LSD.Module.Command = new Class({
  ...
  getCommand: function(force) {
    ...
    var type = this.options.command.type, command;
    ...
    if (this.attributes.command && this.document.commands) command = this.document.commands[this.attributes.command];
...
LSD.Widget.Menu.Context.Button
  = LSD.Widget.Menu.Context.Option
  = LSD.Widget.Menu.Context.Command;
  
LSD.Widget.Menu.Context.Option.Radio
  = LSD.Widget.Menu.Context.Command.Command 
  = LSD.Widget.Menu.Context.Command.Checkbox 
  = LSD.Widget.Menu.Context.Command.Radio 
  = LSD.Widget.Menu.Context.Command;
